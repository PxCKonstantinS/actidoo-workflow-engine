...
def register_data_model(
    *,
    ...
    If *api* is provided, the model must use ``DataModelMixin`` (so it has a stable
    ``id``); versioning (``VersionedMixin``) is optional.
    """

    def decorator(model_class: type) -> type:
        if api is not None and not issubclass(model_class, DataModelMixin):
            raise TypeError(
                f"Data model '{name}' provides an api config but does not use DataModelMixin. "
                f"Only data models (with a stable id) can be exposed via the API.",
            )

        if issubclass(model_class, DataModelMixin):
            # ``title`` is the reserved record title from DataModelMixin. A model
            # declaring its own ``title`` column wins via MRO and silently shadows
            # it (different type/length, not projected/searched as intended) —
            ...
            if not title_col.info.get("wfe_reserved_title"):
                raise TypeError(
                    ...
                    f"which shadows the reserved record title from DataModelMixin (String(200), "
